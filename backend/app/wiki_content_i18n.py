...
Mutated children's `irregularity` judged at `adult_age`: `≥euthanasia_threshold` → consumed (`euthanasia`), else demoted to Soldier. `CASTE_TRAITS` (`entities.py:40`) gives lifespan/speed/sight_mult/fertility. BC maps $asymmetry$ → $irregularity$ for same gate.
...
## Entities (in-memory)
- `Creature` (`entities.py:122`): `id`, `x`, `y`, `angle`, `shape` polygon|line, `sides`, `caste`, `radius`, `age`, `lifespan`, `stage` infant|juvenile|adult|elder, `irregularity`, `health` 0–100, `infected`, `sex` male|female, `mother_id`/`father_id`, `clan_id`/`clan_color`, `sleeping`/`indoors`, `generation`, `born_tick`, `energy`, `status` hungry/starving, `meals`.
- `Food` (`entities.py:257`): `x`, `y`, `growth` 0–1.
- `Corpse` (`entities.py:277`): `x`, `y`, `ttl`, `energy`.
- `House` (`entities.py:286`): `x`, `y`, `size`, `door_width`/`door_side`/`door_offset`, `clan_id`/`clan_color` (settlement), `is_ruin`/`abandoned_ticks`, `takeover_tick` (last hostile takeover, §AT-3 render flash); ruin after `house_decay_ticks` (`config.py:217`), `settlement`/`ruin`/`takeover` events.
...
- `StateMessage` (`protocol.py:98`): `tick`, `seed`, `width`/`height`/`boundary`, `population`, `entities`, `clans`, `creatures_alive`/`creatures_dead`/`dead_by_cause`, `infected_count`, `time_of_day`/`day`/`season`/`weather`, `terrain_fertile`/`terrain_rocks`, `relations`, `events`, plus periodic 1 Hz `analytics` frame (coalesced macro metrics).
- `HistoryEvent` (`protocol.py:184`): `type` death|birth|promotion|demotion|outbreak|recovery|bloom|alliance|rivalry|predation|war|ruin|settlement, `tick`, `entity_id`, `caste`, `cause`, `x`/`y`, `payload` (parents/sides/generation/clan_id etc).
- `HelloMessage` (`protocol.py:206`): `seed`, `tick_rate`, `width`/`height`/`boundary`.
...
- `Food` (`entities.py:257`): Vị trí `x`, `y`, độ chín sinh trưởng `growth` từ 0.15 đến 1.0.
- `Corpse` (`entities.py:277`): Vị trí `x`, `y`, thời gian phân hủy còn lại `ttl`, lượng năng lượng cung cấp `energy`.
- `House` (`entities.py:286`): Vị trí `x`, `y`, kích thước nhà `size`, độ rộng cửa `door_width`, cạnh đặt cửa `door_side`, bang tộc sở hữu `clan_id`/`clan_color`, dấu hiệu phế tích `is_ruin`, số tick bị bỏ hoang `abandoned_ticks`, thời điểm bị chiếm đoạt `takeover_tick`.
...
- `StateMessage` (`protocol.py:98`): Thông điệp trạng thái toàn thế giới gửi mỗi tick gồm: số tick `tick`, hạt giống `seed`, kích thước bản đồ `width`/`height`/`boundary`, tổng dân số `population`, danh sách thực thể `entities`, thông tin bang phái `clans`, số sinh vật sống/chết và nguyên nhân tử vong, số ca nhiễm bệnh, thời điểm trong ngày/ngày/mùa/thời tiết, địa hình, ma trận quan hệ ngoại giao, danh sách biến cố mới phát sinh, kèm khung dữ liệu `analytics` định kỳ 1 Hz phục vụ Đài thiên văn.
- `HistoryEvent` (`protocol.py:184`): Bản ghi sự kiện lịch sử gồm: loại sự kiện `type` (tử vong, sinh nở, thăng hạng, giáng cấp, bùng dịch, bình phục, cây nở hoa, liên minh, thù địch, săn mồi, tuyên chiến, phế tích, lập ấp), số tick xảy ra, mã thực thể liên quan, đẳng cấp, nguyên nhân, tọa độ và gói dữ liệu bổ trợ `payload`.
- `HelloMessage` (`protocol.py:206`): Thông điệp chào ban đầu khi kết nối WebSocket thành công: hạt giống `seed`, tốc độ tick `tick_rate`, giới hạn thế giới `width`/`height`/`boundary`.
...
- `Creature` (`entities.py:122`) : Identifiant `id`, position `x`, `y`, orientation `angle`, morphologie `shape` (`polygon` ou `line`), nombre de côtés `sides`, caste sociale `caste`, rayon `radius`, âge `age`, espérance de vie `lifespan`, stade biologique `stage` (`infant`, `juvenile`, `adult`, `elder`), asymétrie `irregularity`, vitalité `health` (0–100), infection `infected`, sexe `sex`, filiation `mother_id`/`father_id`, appartenance `clan_id`/`clan_color`, état de repos `sleeping`/`indoors`, génération généalogique `generation`, tick de naissance `born_tick`, réserve d'énergie `energy`, statut de faim `status`.
- `Food` (`entities.py:257`) : Coordonnées `x`, `y`, stade de maturation végétale `growth` (de 0.15 à 1.0).
- `Corpse` (`entities.py:277`) : Coordonnées `x`, `y`, durée de décomposition restante `ttl`, énergie nutritive `energy`.
- `House` (`entities.py:286`) : Coordonnées `x`, `y`, superficie `size`, largeur de porte `door_width`, clan propriétaire `clan_id`/`clan_color`, statut de ruine `is_ruin`, dernier assaut hostile `takeover_tick`.
...
- `StateMessage` (`protocol.py:98`) : Données complètes diffusées à chaque tick : numéro de `tick`, graine `seed`, dimensions `width`/`height`/`boundary`, population totale, ensemble des entités actives, clans, statistiques de mortalité, horloge nycthémérale, saison, météo, matrice diplomatique, événements récents et charge utile `analytics` à 1 Hz pour l'Observatoire.
- `HistoryEvent` (`protocol.py:184`) : Événement chronologique typé : décès, naissance, promotion, épidémie, rémission, floraison, alliance, guerre, assaut de colonie avec horodatage en tick et charge utile `payload`.
- `HelloMessage` (`protocol.py:206`) : Message d'accueil émis à l'ouverture du WebSocket : graine, cadence de tick, et limites du monde.
